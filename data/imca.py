...
def gen_ivae_data(Ncomp=10, Nlayer=2, Nsegment=20, NsegmentObs=200, Nmodality=2, source='Gaussian', seed=1):
    ...
    for i in range(Nsegment):
        ...
        var1[ic, ic] = 8
        var2[ic, ic] = 8
        crosscov = correlation*np.sqrt(var1*var2)
        cov = np.hstack([np.vstack([var1, crosscov]), np.vstack([crosscov, var2])])

        # TODO update code in a more general case of M modalities
        # var_list = []
        # crosscov = 1
        # for m in range(Nmodality):
        #     var = np.diag(np.random.uniform(0.2, 4, Ncomp))
        #     var_list.append(var)
        ...
        segID = range(NsegmentObs * i, NsegmentObs * (i + 1))
        labels[segID] = i

    dat = np.concatenate(data_list, axis=0)

    mixedDat = np.copy(dat) # 4000 x 10 x 2
    for m in range(Nmodality):
        for l in range(Nlayer - 1):
            # generate causal matrix first:
            A = ortho_group.rvs(Ncomp) # generateUniformMat( Ncomp, condThresh )
            ...
            # we first apply non-linear function, then causal matrix!
            if NonLin == 'leaky':
                mixedDat[:,:,m] = leaky_ReLU(mixedDat[:,:,m], negSlope)
            elif NonLin == 'sigmoid':
                mixedDat[:,:,m] = sigmoidAct(mixedDat[:,:,m])
            # apply mixing:
            mixedDat[:,:,m] = np.dot(mixedDat[:,:,m], A)

    return {'source': dat, 'obs': mixedDat, 'labels': labels, 'mixing': mixingList}
